...
public sealed class ModuleHostTests
{
    ...
    {
        private readonly string? old = Environment.GetEnvironmentVariable("GORT_DATA_ROOT");
        public string Root { get; } = Path.Combine(Directory.GetCurrentDirectory(), "TestData", Guid.NewGuid().ToString("N"));
        public DataRootScope() => Environment.SetEnvironmentVariable("GORT_DATA_ROOT", Root);
        public ValueTask DisposeAsync()
        {
            Environment.SetEnvironmentVariable("GORT_DATA_ROOT", old);
