...
FILE=/tmp/bigtest.bin
TOTAL=6291456
CHUNK=4194304

echo "=== chunk 1: bytes 0-4194303 ==="
...
echo "=== chunk 2: bytes 4194304-6291455 ==="
...
echo "=== finalize ==="
curl -s -X POST "$BASE/api/files/uploads/$SID/finalize" \
  -H "Authorization: Bearer $TOKEN" --max-time 15
echo ""

echo "=== verify uploaded file sha256 ==="
sha256sum /srv/nas/bigtest.bin | cut -d' ' -f1
