...
/// FortOS does not start its own share daemons to avoid port and state file conflicts with distribution systemd units.
/// </summary>
public sealed class ShareServiceCoordinator
{
    ...
    public ShareServiceCoordinator(IServiceRegistry? registry, IServiceSupervisor? supervisor, IProcessManager? processManager, IFortOSConfiguration? configuration, ILogger logger)
    {
        _registry = registry;
