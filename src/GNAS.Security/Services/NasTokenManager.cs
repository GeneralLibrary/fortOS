...
public sealed class NasTokenManager : ITokenManager
{
    ...
    /// <inheritdoc />
    public async Task<GNAS.Core.TokenValidationResult> ValidateTokenAsync(string token, CancellationToken ct)
    {
...
""";
    ...
    private static GNAS.Core.TokenValidationResult Failed(string message, Exception? exception = null, string? jti = null) => new()
    {
        IsValid = false,
