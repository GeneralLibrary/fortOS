...
namespace GORT.Security.KeyStore;
...
public sealed class NasKeyStore : INasKeyStore, IMasterKeyRotationService
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("GORT_MASTER_KEY")))
            throw new ConfigurationException("Cannot rotate master key while GORT_MASTER_KEY environment variable is set.");
        await _sync.WaitAsync(ct).ConfigureAwait(false);
        try
        {
            EnsureDirectory();
    ...
    private async Task<byte[]> GetMasterKeyCoreAsync(CancellationToken ct)
    {
        ...
        EnsureDirectory();
        var env = Environment.GetEnvironmentVariable("GORT_MASTER_KEY");
        if (!string.IsNullOrWhiteSpace(env))
        {
            var key = Convert.FromBase64String(env);
            if (key.Length != 32)
            {
                throw new ConfigurationException("GORT_MASTER_KEY must be a base64-encoded 32-byte key.");
            }
