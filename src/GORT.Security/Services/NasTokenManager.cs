...
public sealed class NasTokenManager : ITokenManager
{
    ...
    public NasTokenManager(INasKeyStore keyStore, IDatabaseProvider database, IGortConfiguration? configuration = null)
    {
        _keyStore = keyStore;
        _database = database;
    ...
    /// <inheritdoc />
    public async Task<GORT.Core.TokenValidationResult> ValidateTokenAsync(string token, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var expectedDevice = _configuration?.GetValue("security:device_binding") ?? Environment.GetEnvironmentVariable("GORT_DEVICE_BINDING");
            ...
            return new GORT.Core.TokenValidationResult
            {
                IsValid = true,
                Payload = payload,
                Jti = payload.Jti,
...
""";
    ...
    private static GORT.Core.TokenValidationResult Failed(string message, Exception? exception = null, string? jti = null) => new()
    {
        IsValid = false,
