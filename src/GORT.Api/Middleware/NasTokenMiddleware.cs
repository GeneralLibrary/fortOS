...
public sealed class NasTokenMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, ITokenManager tokenManager, IDatabaseProvider database, IConfiguration configuration)
    {
        ...
        {
            if (context.GetEndpoint()?.Metadata.GetMetadata<GORT.Api.Authorization.BootstrapOnlyAttribute>() is not null && await NoUsersExistAsync(database, context.RequestAborted).ConfigureAwait(false))
            {
