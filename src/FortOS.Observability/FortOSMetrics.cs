...
public sealed class FortOSMetrics : IDisposable
{
    private readonly Meter _meter = new("FortOS");
    ...
    public FortOSMetrics()
    {
        _httpRequests = _meter.CreateCounter<long>("fortos_http_requests_total");
        _httpErrors = _meter.CreateCounter<long>("fortos_http_errors_total");
        _httpDuration = _meter.CreateHistogram<double>("fortos_http_request_duration_seconds");
        _backupSuccess = _meter.CreateCounter<long>("fortos_backup_success_total");
        _backupFailure = _meter.CreateCounter<long>("fortos_backup_failure_total");
    }
    ...
    public void RecordHttp(string method, int status, double seconds)
    {
        ...
        Increment($"fortos_http_requests_total{{method=\"{method}\",status=\"{status}\"}}");
        if (status >= 500) { _httpErrors.Add(1, tags); Increment($"fortos_http_errors_total{{method=\"{method}\",status=\"{status}\"}}"); }
    }

    public void RecordBackup(bool success, double seconds, double freshnessHours = 0)
    {
        if (success) { _backupSuccess.Add(1); Increment("fortos_backup_success_total"); }
        else { _backupFailure.Add(1); Increment("fortos_backup_failure_total"); }
        _values["fortos_backup_duration_seconds"] = seconds;
        _values["fortos_backup_freshness_hours"] = freshnessHours;
    }

    public void RecordAgentHealth(bool healthy) => _values["fortos_agent_health"] = healthy ? 1 : 0;
    public void RecordAgentRestart() => Increment("fortos_agent_restarts_total");
    public void RecordProtocolHealth(string protocol, bool healthy) => _values[$"fortos_protocol_health{{protocol=\"{protocol}\"}}"] = healthy ? 1 : 0;
    ...
    {
        var name = "fortos_" + metric.MetricName.Replace('.', '_').Replace('-', '_');
