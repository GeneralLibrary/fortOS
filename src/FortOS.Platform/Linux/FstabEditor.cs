...
/// Provides a pure-functional entry add/remove, guaranteeing idempotence: only one FortOS-managed record per mount point,
/// and does not touch existing entries not managed by FortOS (root partition, swap, etc.).
/// </summary>
public static class FstabEditor
{
    /// <summary>Line-end marker for FortOS-managed entries, used to distinguish manually maintained entries.</summary>
    public const string ManagedMarker = "# fortos-managed";

    /// <summary>
    /// Inserts or updates a mount record.
    /// Replaces if a FortOS-managed entry already exists for the same mount point, otherwise appends to the end of the file.
    /// </summary>
    ...
    /// <summary>
    /// Removes the FortOS-managed record for the specified mount point; unmanaged entries remain unchanged.
    /// </summary>
    /// <param name="content">Existing fstab content.</param>
    /// <param name="mountPoint">Mount point.</param>
    /// <returns>Updated fstab content.</returns>
