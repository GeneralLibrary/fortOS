...
{
    private const string ManifestName = ".gort-checksums.json";
    ...
    private static string? CreateCheckpointIfNeeded(string target, string runId)
    {
        ...
        var checkpoint = target.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) + $".gort-checkpoint-{runId}";
    ...
    private static Task ApplyRetentionAsync(BackupTask task, string target, CancellationToken ct)
    {
        ...
        foreach (var checkpoint in Directory.EnumerateDirectories(target + ".gort-checkpoint-*"))
