...
public sealed class TuiRenderer
{
    ...
    public async Task<int> RunAsync(FortOSApiClient client, CancellationToken cancellationToken = default)
    {
        try
        {
            await AnsiConsole.Live(new Panel("Loading FortOS...")).AutoClear(false).StartAsync(async ctx =>
            {
                var quit = false;
    ...
    private async Task<IRenderable> SafeRenderAsync(FortOSApiClient client, CancellationToken cancellationToken)
    {
        try { return await _pages[_index].RenderAsync(client, cancellationToken); }
        catch (FortOSApiException ex) { return new Panel(Markup.Escape(ex.Message)) { Header = new PanelHeader("Cannot connect") }; }
