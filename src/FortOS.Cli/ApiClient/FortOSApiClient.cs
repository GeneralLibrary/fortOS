...
public sealed class FortOSApiException : Exception
{
    /// <summary>HTTP status code; null on connection failure.</summary>
    public HttpStatusCode? StatusCode { get; }

    /// <summary>Creates an API exception.</summary>
    public FortOSApiException(string message, HttpStatusCode? statusCode = null, Exception? inner = null) : base(message, inner) => StatusCode = statusCode;
}

/// <summary>Encapsulates FortOS REST API with authentication retry logic.</summary>
public sealed class FortOSApiClient : IDisposable
{
    ...
    /// <summary>Creates a FortOS REST client.</summary>
    public FortOSApiClient(string? server = null, string? token = null, HttpClient? httpClient = null)
    {
        _store = AuthStore.Load();
        var baseUrl = server ?? Environment.GetEnvironmentVariable("FortOS_SERVER") ?? _store.Server ?? "http://localhost:5000";
        if (!baseUrl.EndsWith('/')) baseUrl += "/";
        _explicitToken = token ?? Environment.GetEnvironmentVariable("FortOS_TOKEN");
    ...
    public async IAsyncEnumerable<string> GetSseStreamAsync(string path, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new FortOSApiException($"Cannot connect to FortOS server {Server}, please verify the service is running.", null, ex);
        }
    ...
    private async Task<HttpResponseMessage> SendOnceAsync(HttpMethod method, string path, object? body, CancellationToken cancellationToken)
    {
        ...
        {
            throw new FortOSApiException($"Connection to FortOS server {Server} timed out.", null, ex);
        }
    }
    ...
    private static async Task<FortOSApiException> BuildErrorAsync(HttpResponseMessage response, CancellationToken cancellationToken)
    {
        var text = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
        var message = $"FortOS API request failed: {(int)response.StatusCode} {response.ReasonPhrase}";
        try
        {
