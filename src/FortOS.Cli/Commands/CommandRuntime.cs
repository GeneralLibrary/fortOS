...
public static class CommandRuntime
{
    ...
    public static FortOSApiClient Client(ParseResult result, CliOptions options) => new(result.GetValue(options.Server), result.GetValue(options.Token));
    ...
    public static async Task<int> RunWithClientAsync(
        ParseResult result,
        CliOptions options,
        Func<FortOSApiClient, CancellationToken, Task<int>> action,
    ...
    {
        // If either stdin or stdout is redirected (pipe/script), skip interactive login: interactive prompts need a
        // real terminal, otherwise Spectre throws "Failed to read input in non-interactive mode".
        ...
        if (!string.IsNullOrWhiteSpace(explicitToken) || !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("FortOS_TOKEN")))
        {
            return false;
        }
    ...
    public static async Task<int> RunMessageAsync(ParseResult result, CliOptions options, Func<FortOSApiClient, CancellationToken, Task<JsonDocument>> action, string success, CancellationToken cancellationToken = default)
    ...
    public static void RenderGeneric(JsonDocument doc)
    {
        ...
        var panel = new Panel(new Text(doc.RootElement.ToString())) { Header = new PanelHeader("FortOS") };
