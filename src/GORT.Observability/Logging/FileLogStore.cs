...
namespace GORT.Observability.Logging;
...
public sealed class FileLogStore : ILogStore
{
    ...
    public FileLogStore(IGortConfiguration? configuration = null, string? dataRoot = null, long maxShardBytes = DefaultShardBytes)
    {
        var root = dataRoot ?? configuration?.GetValue("logging:data_root") ?? Environment.GetEnvironmentVariable("GORT_DATA_ROOT") ?? "/srv/nas";
