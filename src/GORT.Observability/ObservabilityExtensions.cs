using GORT.Core;
using GORT.Observability.Alerts;
using GORT.Observability.Alerts.Notifiers;
using GORT.Observability.Audit;
...
public static class ObservabilityExtensions
{
    ...
    {
        services.AddSingleton<GortMetrics>();
...
public sealed class LazySerilogBootstrapper : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        Log.Logger = new LoggerConfiguration()
            .WriteTo.Async(sink => sink.Sink(new GortSerilogSink(_pipeline)))
            .CreateLogger();
