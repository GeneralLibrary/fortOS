...
using GORT.Core;
using GORT.Observability.Alerts.Notifiers;
...
namespace GORT.Observability.Alerts;
...
public sealed class AlertEngine : IAlertEngine, IHostedService, IDisposable
{
    ...
    public AlertEngine(
        IGortConfiguration configuration,
    ...
    private string GetConfigDirectory()
        => _configuration.GetValue("config:dir") ?? Environment.GetEnvironmentVariable("GORT_CONFIG_DIR") ?? Path.Combine(Environment.GetEnvironmentVariable("GORT_DATA_ROOT") ?? "/srv/nas", "config");
    ...
    [
        SloRule("slo-backup-freshness", "Backup freshness exceeds threshold", "warning", "gort_backup_freshness_hours", "gt", 24),
        SloRule("slo-backup-failures", "Backup failure", "critical", "gort_backup_failure_total", "gt", 0),
        SloRule("slo-protocol-health", "Share protocol unavailable", "critical", "gort_protocol_health", "lt", 1),
        SloRule("slo-agent-restarts", "Agent restart storm", "warning", "gort_agent_restarts_total", "gt", 5),
        SloRule("slo-http-errors", "HTTP 5xx errors", "warning", "gort_http_errors_total", "gt", 0),
    ...
    private static AlertRule SloRule(string id, string name, string severity, string metric, string op, double value, int? durationSeconds = null) => new()
    {
        RuleId = id,
        Name = name,
        Description = $"GORT default SLO rule: {name}",
        Severity = severity,
    ...
    private static AlertRule EventRule(string id, string name, string severity, string topic, int count, int withinSeconds) => new()
    {
        RuleId = id,
        Name = name,
        Description = $"GORT default event rule: {name}",
        Severity = severity,
