...
/// <summary>
/// 主窗口 VM:组装七个向导页,负责把页面输入编译为 InstallConfig 并驱动执行。
/// 依赖通过工厂注入,便于无头测试。
/// </summary>
public partial class MainWindowViewModel : ViewModelBase
{
        ...
        Action<Action>? uiDispatch = null)
    {
        ...
        Confirm = new ConfirmViewModel(Welcome, DiskLayout, Network, Account);
        Install = new InstallViewModel(sessionFactory ?? (() => InstallerSession.CreateDefault()), uiDispatch);
        Complete = new CompleteViewModel();

        Wizard = new WizardViewModel([Welcome, DiskLayout, Network, Account, Confirm, Install, Complete]);
    ...
    /// <summary>确认页「开始安装」:前进到执行页并启动引擎。</summary>
    [RelayCommand]
    private async Task BeginInstallAsync()
    {
        // 防御:JumpTo 可绕过向导校验链,进入执行前必须确认目标盘已选。
        if (!Wizard.CanGoNext || DiskLayout.SelectedSystemDisk is null)
        {
            return;
        }
        Wizard.NextCommand.Execute(null); // Confirm → Install
