...
namespace GORT.Agent.Collector;
...
public sealed class AgentLogCollector : BackgroundService
{
    ...
    private async Task PullContainerLogsAsync(string agentId, CancellationToken ct)
    {
        ...
        using var process = CreateDockerProcess($"logs --since {since:O} --timestamps gort-{agentId}");
        try
        {
            process.Start();
    ...
    private async Task WarnDockerUnavailableAsync(CancellationToken ct)
    {
        ...
        await _logPipeline.ProcessAsync(new LogEntry
        {
            ...
            SourceComponent = "GORT.Agent.AgentLogCollector",
    ...
    private static (string? AgentId, string? Name) ExtractAgentIdentity(JsonElement root)
    {
        ...
        var label = TryGetAttribute(root, "gort.agent.id") ?? TryGetAttribute(root, "com.gort.agent.id");
        if (!string.IsNullOrWhiteSpace(label))
        {
            return (label, name);
        }

        if (!string.IsNullOrWhiteSpace(name) && name.StartsWith("gort-", StringComparison.OrdinalIgnoreCase))
        {
            return (name[5..], name);
        }
