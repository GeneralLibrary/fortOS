...
public sealed class TuiRenderer
{
    ...
    public async Task<int> RunAsync(GortApiClient client, CancellationToken cancellationToken = default)
    {
        try
        {
            await AnsiConsole.Live(new Panel("Loading GORT...")).AutoClear(false).StartAsync(async ctx =>
            {
                var quit = false;
    ...
    private async Task<IRenderable> SafeRenderAsync(GortApiClient client, CancellationToken cancellationToken)
    {
        try { return await _pages[_index].RenderAsync(client, cancellationToken); }
        catch (GortApiException ex) { return new Panel(Markup.Escape(ex.Message)) { Header = new PanelHeader("Cannot connect") }; }
