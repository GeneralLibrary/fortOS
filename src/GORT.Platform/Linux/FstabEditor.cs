...
/// Provides a pure-functional entry add/remove, guaranteeing idempotence: only one GORT-managed record per mount point,
/// and does not touch existing entries not managed by GORT (root partition, swap, etc.).
/// </summary>
public static class FstabEditor
{
    /// <summary>Line-end marker for GORT-managed entries, used to distinguish manually maintained entries.</summary>
    public const string ManagedMarker = "# gort-managed";

    /// <summary>
    /// Inserts or updates a mount record.
    /// Replaces if a GORT-managed entry already exists for the same mount point, otherwise appends to the end of the file.
    /// </summary>
    ...
    /// <summary>
    /// Removes the GORT-managed record for the specified mount point; unmanaged entries remain unchanged.
    /// </summary>
    /// <param name="content">Existing fstab content.</param>
    /// <param name="mountPoint">Mount point.</param>
    /// <returns>Updated fstab content.</returns>
