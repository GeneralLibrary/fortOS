...
/// when GORT internal users (SQLite) are created or deleted, so that sharing protocol clients
/// can authenticate with the same credentials.
/// Implementations must be best-effort: provisioning failure should not block the GORT user lifecycle.
/// </summary>
