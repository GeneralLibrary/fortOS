...
public static class RecycleCommand
{
    ...
    public static Command Create(CliOptions options)
    {
        ...
        var restore = new Command("restore", "还原项目") { shareArg, idArg };
        restore.SetAction((p, ct) => CommandRuntime.RunAsync(p, options, (c, t) => c.PostAsync($"api/recycle/{Uri.EscapeDataString(p.GetRequiredValue(shareArg))}/restore/{Uri.EscapeDataString(p.GetRequiredValue(idArg))}", null, t), cancellationToken: ct));
        ...
        empty.SetAction((p, ct) => CommandRuntime.RequireConfirm(p.GetValue(confirm)) ? CommandRuntime.RunAsync(p, options, (c, t) => c.DeleteAsync($"api/recycle/{Uri.EscapeDataString(p.GetRequiredValue(emptyShare))}/empty", t), cancellationToken: ct) : Task.FromResult(2));
