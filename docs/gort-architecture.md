# GORT — New Generation Linux NAS System Technical Architecture

> **Project Codename**: GORT  
...
║                                                                            ║
║  ┌──────────────────────────────────────────────────────────────────┐    ║
║  │  PRESENTATION       Desktop CLI Tool (gort) │ Web Dashboard*       │    ║
║  └────────────────────────────┬─────────────────────────────────────┘    ║
║                               │ HTTPS / REST API                          ║
║  ┌────────────────────────────┴─────────────────────────────────────┐    ║
...
GORT's presentation layer includes the Linux command-line tool `gort`; all management operations are performed through the CLI.
...
│  │  · Local config file management (~/.gort/config)       │ │
│  └────────────────────────┬───────────────────────────────┘ │
│                           │                                  │
│  ┌────────────────────────┴───────────────────────────────┐ │
...
Running `gort` without subcommands enters the interactive terminal UI:

```text
┌──────────────────────────────────────────────────────────┐
│  GORT v1.0.0                          nas://home-nas     │
│──────────────────────────────────────────────────────────│
│                                                          │
...
```bash
# Query
gort status                    # System status overview
gort disk list                 # Disk list
gort share list                # Shared folder list
gort service list              # Service list
gort agent list                # Agent list

# Operations
gort share create media /mnt/nas/data/media
gort service restart smb-daemon
gort agent deploy openclaw --capabilities "storage:share:media:read"
gort agent start openclaw

# Logs and monitoring
gort log view --follow --category agent --agent openclaw
gort log query --level error --last 1h
gort audit verify              # Verify audit chain integrity
gort alert list --severity warning

# Output format
gort disk list --output json   # JSON output (pipeline-friendly)
gort disk list --output table  # Table output (default)
gort disk list --no-color      # Disable ANSI colors
```

### 3.3 Built-in Web Dashboard (Optional)

CLI is the primary interaction method. For scenarios requiring graphical monitoring, GORT can optionally enable a lightweight embedded Web Dashboard:
...
syntax = "proto3";
package gort.storage;
option csharp_namespace = "GORT.Proto.Storage";
...
syntax = "proto3";
package gort.share;
option csharp_namespace = "GORT.Proto.Share";
...
syntax = "proto3";
package gort.agent;
option csharp_namespace = "GORT.Proto.Agent";
...
syntax = "proto3";
package gort.servicebus;
option csharp_namespace = "GORT.Proto.ServiceBus";
...
syntax = "proto3";
package gort.audit;
option csharp_namespace = "GORT.Proto.Audit";
...
syntax = "proto3";
package gort.common;
option csharp_namespace = "GORT.Proto.Common";
...
All Proto files are compiled through `Grpc.Tools` MSBuild integration, with generated code placed under the `GORT.Proto` namespace.
...
│       notification                                      │
│     · CLI prompt: gort status shows red disk status     │
│                                                         │
...
smb:
  workgroup: WORKGROUP
  server_string: "GORT File Server"
  netbios_name: gort-nas
...
```
GORT Permission Layer     SMB/NFS               POSIX/Linux
─────────────────────────────────────────────────────────────
...
· Managed via CLI:
  gort recycle list <share>         # View recycle bin contents
  gort recycle restore <id>         # Restore specified file
  gort recycle empty <share>        # Empty recycle bin
  gort recycle config <share>       # Configure recycle bin policy
```
...
│  · gort snapshot create <dataset>                       │
│  · gort snapshot list <dataset>                         │
│  · gort snapshot restore <dataset> <snapshot-id>        │
└─────────────────────────────────────────────────────────┘
...
│              │              │                │ retent. │
│                                                         │
│  Snapshot naming: gort-{dataset}-{yyyyMMdd-HHmmss}      │
...
│  shadow:sort = desc                                     │
│  shadow:format = gort-{dataset}-%Y%m%d-%H%M%S           │
│                                                         │
...
│  ├── Target: S3 / Backblaze B2 / remote GORT node       │
...
│                                                         │
│  1. Install GORT system (ISO / script)                  │
│  2. Run recovery wizard: gort recovery start            │
│  3. Select recovery source:                             │
│     · Local backup disk → auto-mount → scan backup      │
│       directory                                         │
│     · Cloud storage → enter credentials → list          │
│       available backups                                 │
│     · Remote GORT → enter address + credentials         │
...
```
HarmonyOS Concept        →    GORT Adaptation
...
This layer is GORT's core innovation — a Linux application-level service manager that uniformly manages all service processes in the NAS (native services + Docker containers).
...
|------|------|------|
| **Interaction** | .NET 10 Console Application (gort CLI) | Linux CLI tool, supports TUI + batch processing |
...
```yaml
# docker-compose.yml — GORT can also be containerized
version: '3.8'

services:
  gort-core:
    image: gort/core:latest
    container_name: gort-core
    ...
    volumes:
      ...
      - /dev:/host/dev
    environment:
      - GORT_CONFIG_PATH=/srv/nas/config/nas.yaml
      - GORT_DATA_ROOT=/srv/nas/data
      - ASPNETCORE_URLS=http://0.0.0.0:5000
      - ASPNETCORE_ENVIRONMENT=Production

  gort-loki:
    image: grafana/loki:latest
    container_name: gort-loki
    restart: unless-stopped
    volumes:
...
|------|----------|------|
| **ISO Image Install** | x64 bare metal | Based on Debian 12 Live ISO, includes Debian Installer and all GORT dependencies |
| **Script Install** | Existing Linux system | `curl -fsSL https://get.gort.io | bash` one-click install |
| **Docker Self-Deploy** | Dev/test/lightweight | Run GORT Core containerized, mount Docker Socket |
...
─────────────────
  Bare metal?
    ...
    │        · Download gort-debian12-{version}-amd64.iso
    ...
    └── No → Docker environment already available?
              ...
              │        · wget https://get.gort.io/docker-compose.yml
              │        · docker compose up -d
              │
              └── No → Script install
                       · curl -fsSL https://get.gort.io | bash
                       · Auto-detect platform → install .NET Runtime +
                         dependencies → deploy GORT
```
...
publishes the GORT API and CLI as `linux-x64` self-contained applications, then writes them via `live-build` into
the Live root filesystem, enabling Debian Installer, Docker Compose v2, and `gort.service`.
Build artifacts and their SHA-256 checksum files are located in `artifacts/iso/`; the
`GORT Debian ISO` workflow in GitHub Actions can be triggered manually and is automatically built at Release time.

```
┌─────────────────────────────────────────────────────────┐
│  GORT Installer (TUI Setup Wizard)                     │
│                                                         │
...
│  │  Hostname:   [gort-nas_____________]              │ │
│  └───────────────────────────────────────────────────┘ │
│                                                         │
...
│  │  Timezone:    Asia/Shanghai                       │ │
│  │                                                    │ │
│  │  After installation, visit http://gort-nas:5000   │ │
│  │                                                    │ │
...
┌─────────────────────────────────────────────────────────┐
│  Welcome to GORT v1.0.0!                                │
│                                                         │
│  It looks like this is your first time running GORT.    │
...
────────────────
  ...
  ┌─────────────────────────────────────────────────────┐
  │  GORT Initialization Complete!                      │
  │                                                     │
  │  System Information:                                │
  │    Hostname:   gort-nas                             │
  │    Address:    http://gort-nas:5000 (Management API)│
  │    Dashboard:  http://gort-nas:5000/dashboard       │
  │    Storage:    pool-main (4TB, RAID 1, Btrfs)       │
  │    Shares:     media, documents, downloads          │
  │                                                     │
  │  Run 'gort' to enter interactive TUI management     │
  │  Run 'gort help' to view all available commands     │
  └─────────────────────────────────────────────────────┘
```

### 14.4 Configuration Initialization Details

On first boot, GORT automatically generates initial configuration:
...
```bash
gort ups status                  # UPS status (charge/load/remaining time)
gort ups list                    # List connected UPS devices
gort ups test                    # Trigger UPS self-test
gort ups config set             # Configure UPS parameters
```
...
| Dimension | OMV Original | GORT New Architecture |
|------|-----------|-------------|
...
| **Client** | Web UI (Angular/ExtJS) | Desktop CLI Tool (gort) + optional Web Dashboard |
...
| **Trade-off** | ZFS provides more mature data integrity but requires DKMS and higher memory overhead. Advanced users can choose via `gort pool create --fs zfs` |
...
> **Updated**: 2026-07-25  
> **Related Document**: [GORT Implementation Prompts](gort-implementation-prompts.md)
