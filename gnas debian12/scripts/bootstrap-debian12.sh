#!/usr/bin/env bash
# =============================================================================
# GNAS Debian 12 — Offline Package Cache Builder
# =============================================================================
# Downloads ALL binary .deb packages (including transitive dependencies) that
# GNAS needs into cache/debs/. Also downloads Debian source packages into
...
echo "=== GNAS Debian 12 Package Cache Builder ==="
...
download_packages() {
    ...
    local debian_pkgs
    debian_pkgs=$(grep -E "^\[(REQUIRED|INDIRECT)\]" "${CONFIG_DIR}/gnas-packages.list" | \
        awk '{print $2}' | grep -v "^#" | sort -u | \
        grep -v -E '^(containerd\.io|docker-|minio|mc|borgbackup|restic|zfs|prometheus-)' | \
        tr '\n' ' ')

    local future_pkgs
    future_pkgs=$(grep -E "^\[FUTURE\]" "${CONFIG_DIR}/gnas-packages.list" | \
        awk '{print $2}' | grep -v "^#" | sort -u | \
