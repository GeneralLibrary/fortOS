...
本文档详细记录了 GNAS 对 Debian 12 系统和服务的依赖关系，作为裁剪依据。
...
2. **eng/iso/config/package-lists/gnas.list.chroot** — ISO 构建的包列表
3. **eng/iso/config/hooks/live/0100-gnas-runtime.hook.chroot** — 启用的 systemd 服务
4. **eng/iso/build-in-container.sh** — ISO 构建流程
5. **src/GNAS.Platform/Linux/**.cs — 平台层调用的所有 Linux 命令
6. **src/GNAS.Modules.*/*.cs** — 各模块调用的外部命令
7. **src/GNAS.Agent/**.cs — Docker/容器管理依赖
8. **docs/gnas-architecture.md** — 架构文档中引用的服务和命令
...
### 2.1 GNAS.Platform/Linux/LinuxDiskManager.cs
...
### 2.2 GNAS.Platform/Linux/LinuxFileSystem.cs
...
### 2.3 GNAS.Platform/Linux/LinuxProcessManager.cs
...
### 2.4 GNAS.Platform/Linux/LinuxNetworkManager.cs
...
### 2.5 GNAS.Modules.Share (Samba/NFS/FTP)
...
| 被排除的服务/包类别 | 排除原因 |
|--------------------|----------|
| GNOME/KDE/Xfce/LXDE 桌面 | GNAS 是 headless NAS，无需桌面环境 |
| Wayland/X11 显示服务 | 同上 |
| CUPS 打印服务 | NAS 不需要打印 |
| PulseAudio/ALSA 音频 | NAS 不需要音频 |
| Bluetooth 蓝牙 | NAS 不需要蓝牙 |
| Avahi/Bonjour | 企业环境不需要 mDNS |
| Postfix/Exim 邮件服务器 | GNAS 通过外部 SMTP 发送告警 |
